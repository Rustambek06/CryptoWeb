...
def str_to_hex(s: str) -> str:
    # Преобразование строки в шестнадцатеричный формат
    return s.encode('utf-8').hex()

def pad_to_block_size(s: str, block_size: int = 16) -> str:
    # Дополнение строки до размера блока
    pad_len = block_size - (len(s) % block_size)
    padded_s = s + chr(pad_len) * pad_len
    return padded_s

def unpad(s: str) -> str:
    if s and isinstance(s, str):
        pad_len = ord(s[-1])
        return s[:-pad_len] if pad_len < len(s) else ""
    else:
